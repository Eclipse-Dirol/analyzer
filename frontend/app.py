...
st.title("ANALYZER")
st.write("Upload text without diacritics," \
"they will not be recognized and the analysis will break.")
...
if input_file == "Upload file":
    with col2:
        select_input_file = st.radio("select file type", file_list)
if input_file == "Upload file":
	if select_input_file == ".txt":
		with col1:
				...
				with col_1_:
					...
					if show:
						...
						try:
							...
							with col_2_:
								st.write(text)
	elif select_input_file == ".bin":
		st.write("Now this methods is not currently supported.")
	elif select_input_file == ".csv":
		st.write("Now this methods is not currently supported.")
